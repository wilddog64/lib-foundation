...
**SPEC WRITTEN — ASSIGNED to Codex (Claude, 2026-07-06) — CDP must launch Playwright-managed Chromium, not system Chrome (BUG #3 in the v0.4.2 ACG-gate line):** `docs/bugs/2026-07-06-cdp-use-playwright-managed-chromium.md`, branch `feat/v0.4.2`. **Root cause (diagnosed live this session):** `_browser_launch` (`scripts/lib/acg/cdp.sh`) launches system `/Applications/Google Chrome.app`; every ACG node script then `chromium.connectOverCDP`s to it. System Chrome auto-updates independently and drifts past the pinned Playwright's DevTools-protocol support — observed Chrome 150 vs Playwright 1.60 (→ Chromium 148) failing `browserType.connectOverCDP: Protocol error (Browser.setDownloadBehavior): Browser context management is not supported`, dead-ending BOTH `make credential-test PROVIDER=aws` and the `make up` ACG path before the headless gate runs. **Empirically proven this session:** upgrading Playwright to 1.61.1 does NOT durably fix it (same failure — Chrome updates again); launching Playwright's own version-locked Chromium (`require('playwright').chromium.executablePath()` → `…/ms-playwright/chromium-1223/chrome-mac-arm64/Google Chrome for Testing.app/...`, confirmed EXISTS) and connecting over CDP returns `CONNECT_OK contexts=1`. **Fix (3 files):** (1) `cdp.sh` `_browser_launch` resolves+launches the Playwright Chromium, hard-fails with an `npm install` message if absent, NO system-Chrome fallback; (2) repoint profile default `…/k3d-manager/profile`→`pw-profile` in all 3 `cdp.sh` sites + `playwright/lib/output.js` `AUTH_DIR` (the existing profile was written by newer system Chrome; CfT refuses a newer-version profile — a fresh dedicated profile makes the transition clean, first run auto-logs in from Keychain). Commit (exact): `fix(acg): launch Playwright-managed Chromium for CDP instead of system Chrome`. Gates: `shellcheck -S warning scripts/lib/acg/cdp.sh`, `npm run check`, `npm test`, sourcing probe, `CHANGE.md` `[Unreleased] ### Fixed`. **USER acceptance gate (not Codex):** quit any stale system Chrome on `:9222`, then `make credential-test PROVIDER=aws` must launch Chrome-for-Testing, sign in headlessly, extract creds — before any subtree-pull. NOT handed off yet (spec just written). Relates to [[feedback_lib_acg_subtree_discipline]], [[feedback_lib_edits_upstream_first]], [[project_antigravity_migration]].

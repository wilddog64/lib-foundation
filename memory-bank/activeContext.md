...
**feat/v0.4.0 — Phase 1 imported `0a7b6dd` (memory-bank `3f39f9d`), VERIFIED WITH GAPS (2026-06-22)** — absorb lib-acg into lib-foundation as optional module `scripts/lib/acg/`. Phase 0 done (lib-acg `v0.1.9` tagged; migration source SHA `7708ae31`). Decisions locked: v0.4.0 / clean tree-copy / `npm ci`. Spec: `docs/plans/v0.4.0-phase1-import-acg-module.md` (overview: `v0.4.0-absorb-lib-acg.md`). **Core rewiring verified correct** at `0a7b6dd`: runtime tree under `scripts/lib/acg/`, `cdp.sh`/`gcp.sh` guarded `../system.sh` bootstrap + `acg.sh` flattened paths, line-188 dispatcher NOTE left for Phase 2, `package.json` check extended, independent `acg` CI job, docs/CHANGE updated. Codex added a legitimate (but unsolicited) finding doc `docs/issues/2026-06-22-acg-cluster-cidr-audit-allowlist.md` (CloudFormation CIDRs trip `_agent_audit`) — kept. **Two spec omissions found:** (1) `playwright.config.js` never imported (sat at lib-acg root, missed by the `playwright/**` glob) → `test:e2e` + `tests/acg-restart.spec.js` cannot run, CI stays green because jest doesn't need it (latent break); (2) decision locked — `bin/` + `Makefile` belong at the lib-foundation **repo root** (root entry points, module-internal node deps). **Follow-up spec ASSIGNED to Codex:** `docs/plans/v0.4.0-phase1-followup-root-entrypoints.md` (import playwright.config, `git mv` bin/ to root + repoint REPO_ROOT, add root Makefile, fix acg CI shellcheck path, doc touch-ups). Phase 2 = rewire k3d-manager; Phase 3 = archive lib-acg.

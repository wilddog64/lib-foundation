...
**BUG #5 SPEC FILED (Claude, 2026-07-06) — reuse a healthy CDP browser; reclaim a stale/zombie one instead of erroring:** `docs/bugs/2026-07-06-cdp-reclaim-unhealthy-browser-instead-of-error.md`, branch `feat/v0.4.2`. **Diagnosed live this session:** after BUG #4, `make credential-test PROVIDER=aws` still died at `connectOverCDP: … Browser.setDownloadBehavior: Browser context management is not supported`. Root cause is NOT drift — proven: the same CFT binary/version (`Chrome/148.0.7778.96`, `chromium-1223`, Playwright 1.60.0) on the same `pw-profile` FAILS connectOverCDP as a lingering/zombie process (PID 20506, left after the operator closed the CDP window) but SUCCEEDS (`contexts=1`) when freshly launched (PID 34701). BUG #4's guard keys on **profile identity** (`_cdp_profile_in_use`), so it adopts the zombie (profile matches) → the node script dies; and for a genuinely foreign browser it hard-`_err`s, forcing a manual `kill`. **Operator decision: ALWAYS RECLAIM :9222** — reuse if Playwright can drive it, else terminate whatever holds the port and relaunch the managed CFT; never hard-error. **Fix (1 file, cdp.sh):** add `_cdp_connectable` (connectOverCDP connect+close health probe — verified `close()` over CDP disconnects WITHOUT killing a healthy browser, so it is non-destructive) + `_cdp_kill_port_listener` (`lsof -t` TERM→KILL reclaim); rewrite the `_browser_launch` reuse branch to `_cdp_connectable`→reuse / else reclaim + fall-through-to-relaunch, replacing the BUG #4 `_err` profile guard. The health probe becomes the anti-drift protection (a drifted system Chrome fails the probe → reclaimed). `_cdp_profile_in_use` stays (still used by the agent-stop + singleton-lock helpers). Commit message: `fix(acg): reclaim stale CDP browser and reuse healthy one instead of erroring`. **NOT yet handed to Codex / not implemented.** Supersedes BUG #4's reuse-branch guard. Relates to [[feedback_lib_acg_subtree_discipline]], [[feedback_lib_edits_upstream_first]].

...
- [ ] **v0.4.2 BUG #4: credential-test must launch managed browser + `_browser_launch` must reject a foreign CDP browser** — spec `docs/bugs/2026-07-06-credential-test-launch-and-browser-identity-guard.md` (Claude, 2026-07-06). BUG #3 fixed only the *launched* browser; two gaps remain: (A) `_browser_launch` adopts any `:9222` listener without identity check → a stale system Chrome 150 (live: PID 56026 on old `profile` dir) gets adopted → drift; (B) `bin/acg-credential-test` never calls `_browser_launch` (curl-guard + connect-only `_cdp_ensure_acg_session`), so BUG #3's managed launch is unreachable. Fix (cdp.sh + bin/acg-credential-test): hoist `_cdp_profile_dir`, guard reuse branch with `_cdp_profile_in_use` (reject foreign browser via `_err`); route credential-test through `_browser_launch`. Verified `_antigravity_browser_ready` resolves standalone. Commit msg: `fix(acg): route credential-test through _browser_launch and guard CDP browser identity`. **NOT yet implemented — pending Codex handoff.** Immediate manual unblock offered to user (kill PID 56026, launch managed Chromium on `:9222`, re-run).

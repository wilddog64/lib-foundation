...
- [ ] **v0.4.2 BUG: headless session gate never wired into `_browser_launch`** — spec `docs/bugs/2026-07-06-acg-session-gate-not-wired-into-browser-launch.md`. `_cdp_ensure_acg_session`/`acg_session_check.js`/`loginWithPage` shipped in v0.4.1 but no production path calls it; `make up`'s only sign-in (`handleSignIn`) relies on Google-PW autofill that can't fire in the `--password-store=basic` dedicated profile → no sign-in/sandbox/AWS-env → CFN `InvalidClientTokenId`. Fix = call gate on both `_browser_launch` paths (`cdp.sh`), no JS change, fast-fail preserved. NOT yet implemented.

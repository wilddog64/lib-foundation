...
- [ ] **v0.4.2 BUG #3: CDP must launch Playwright-managed Chromium, not system Chrome** — spec `docs/bugs/2026-07-06-cdp-use-playwright-managed-chromium.md` (SPEC WRITTEN, ASSIGNED to Codex, not yet handed off). `_browser_launch` (`cdp.sh`) launches system `/Applications/Google Chrome.app`; `connectOverCDP` fails once Chrome auto-updates past the pinned Playwright's protocol (observed Chrome 150 vs PW 1.60/Chromium 148 → `Browser.setDownloadBehavior: Browser context management is not supported`), dead-ending `make credential-test` + `make up`. Upgrading PW (1.61.1) doesn't durably fix; launching PW's own Chrome-for-Testing (`chromium.executablePath()`) → `CONNECT_OK`. Fix = launch the PW Chromium (hard-fail if absent, no system-Chrome fallback) + repoint profile default `profile`→`pw-profile` in 3 `cdp.sh` sites + `output.js` `AUTH_DIR` (older CfT refuses the newer-Chrome-written profile). Commit: `fix(acg): launch Playwright-managed Chromium for CDP instead of system Chrome`. **USER acceptance:** `make credential-test PROVIDER=aws` after quitting stale system Chrome on `:9222`.

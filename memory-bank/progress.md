...
- [ ] **Phase 1 follow-up: root entry points + missing playwright config** — ASSIGNED to Codex (2026-06-22). Spec: `docs/plans/v0.4.0-phase1-followup-root-entrypoints.md`. (1) import `scripts/lib/acg/playwright.config.js` (sat at lib-acg root, missed by `playwright/**` glob — `test:e2e`/`tests/acg-restart.spec.js` can't run without it; CI stays green because jest doesn't need it); (2) decision: `bin/` + `Makefile` at lib-foundation **repo root** (`git mv` bin scripts to root + repoint `REPO_ROOT` to `../scripts/lib/acg`, add root Makefile delegating node targets, fix `acg` CI job shellcheck path, doc touch-ups). Commit msg: `feat(acg): hoist bin/ + Makefile to repo root, import playwright.config (v0.4.0 Phase 1 follow-up)`.

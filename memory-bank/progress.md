...
- [ ] **PR #32 hardening (Copilot findings, ASSIGNED to Codex)** — spec `docs/bugs/2026-06-22-acg-cdp-env-and-creds-injection-hardening.md`. User elected "fix everything now": all 10 deferred Copilot findings folded into PR #32 before merge. 8 changes — thread `PLAYWRIGHT_CDP_HOST/PORT` through `cdp.sh`/`acg.sh`/`acg_extend.js`/`acg_restart.js`/`acg_session_check.js`; replace `gcp.sh` `source <creds>` with an allowlisted no-eval parser (A03); `output.js` `chmodSync` for existing creds files; fix `vars.sh` stale path comments. Triage doc: `docs/issues/2026-06-22-copilot-pr32-review-findings.md`. After Codex commits: re-run CI, **live smoke = `make credential-test PROVIDER=aws` (regression guard) + GCP extract + CDP-override (`PLAYWRIGHT_CDP_PORT`)**, then reply/resolve all 12 Copilot threads, enforce_admins off, user merges.

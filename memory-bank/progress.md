...
- [ ] **v0.4.2 BUG #2: gate not wired into `bin/acg-credential-test` (`make credential-test` path) + `connectBrowser` locked-profile fallback** — spec `docs/bugs/2026-07-06-acg-session-gate-not-wired-into-credential-test.md`. `bin/acg-credential-test` bypasses `_browser_launch`, so `make credential-test` still runs unauthenticated → `Sign in` page; `connectBrowser` falls back to `launchPersistentContext` on the live CDP profile → `Opening in existing browser session`. Fix = source `cdp.sh` + call `_cdp_ensure_acg_session` after CDP-alive check (standalone sourcing verified); guard the persistent-context fallback. Commit msg `fix(acg): wire _cdp_ensure_acg_session into acg-credential-test + guard connectBrowser CDP fallback`. Assigned to Codex; NOT yet implemented. **Acceptance = user-run `make credential-test PROVIDER=aws` in lib-foundation BEFORE subtree-pull.**

...
- [ ] **v0.4.2 BUG #5: reuse healthy CDP browser; reclaim stale/zombie one instead of erroring** — spec `docs/bugs/2026-07-06-cdp-reclaim-unhealthy-browser-instead-of-error.md`. After BUG #4, `make credential-test PROVIDER=aws` still died at `connectOverCDP: Browser.setDownloadBehavior: Browser context management is not supported` — proven NOT drift (same CFT 148 / `chromium-1223` / PW 1.60.0 on `pw-profile` fails as a lingering zombie PID 20506, succeeds fresh PID 34701). BUG #4's profile-identity guard adopts the zombie and hard-errors on foreign browsers. **Operator decision: ALWAYS RECLAIM :9222.** Fix (cdp.sh only): add `_cdp_connectable` (connectOverCDP health probe, non-destructive close) + `_cdp_kill_port_listener` (`lsof -t` reclaim); rewrite `_browser_launch` reuse branch to reuse-if-driveable / else reclaim+relaunch, replacing the BUG #4 `_err`. Commit msg: `fix(acg): reclaim stale CDP browser and reuse healthy one instead of erroring`. **NOT yet implemented — spec filed, awaiting Codex handoff.** Supersedes BUG #4's reuse guard.

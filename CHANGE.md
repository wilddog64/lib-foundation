...
### Added
- `scripts/lib/system.sh`: `_ensure_agy_cli` — install the standalone Antigravity agent CLI (`agy`) into `~/.local/bin` via the official `curl … | bash` installer; idempotent (no-op if `agy` on PATH or `~/.local/bin/agy` exists), user-scope (no sudo). Distinct from `_ensure_antigravity_ide` (the IDE cask). Covered by 2 mocked BATS tests in `scripts/tests/lib/system.bats`.

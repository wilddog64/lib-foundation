...
function _install_debian_kubernetes_client() {
   ...
   _run_command --prefer-sudo -- mkdir -p /etc/apt/keyrings

   # Download the Kubernetes signing key
   if [[ ! -e "/etc/apt/keyrings/kubernetes-apt-keyring.gpg" ]]; then
      curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.28/deb/Release.key \
         | _run_command --prefer-sudo -- gpg --dearmor -o /etc/apt/keyrings/kubernetes-apt-keyring.gpg
   fi

   # Add the Kubernetes apt repository
   echo "deb [signed-by=/etc/apt/keyrings/kubernetes-apt-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.28/deb/ /" | \
      _run_command --prefer-sudo -- tee /etc/apt/sources.list.d/kubernetes.list > /dev/null

   # Update apt package index
   _run_command --prefer-sudo -- apt-get update -y

   # Install kubectl
   _run_command --prefer-sudo -- apt-get install -y kubectl
...
function _install_redhat_helm() {
  _run_command --prefer-sudo -- dnf install -y helm
}

function _install_debian_helm() {
  # 1) Prereqs
  _run_command --prefer-sudo -- apt-get update
  _run_command --prefer-sudo -- apt-get install -y curl gpg apt-transport-https
   ...
   _run_command -- curl -fsSL https://packages.buildkite.com/helm-linux/helm-debian/gpgkey | \
      ...
      _run_command --prefer-sudo -- tee /usr/share/keyrings/helm.gpg >/dev/null

   # 3) Add the Helm repo (with signed-by, required on 24.04)
   echo "deb [signed-by=/usr/share/keyrings/helm.gpg] https://packages.buildkite.com/helm-linux/helm-debian/any/ any main" | \
   _run_command --prefer-sudo -- tee /etc/apt/sources.list.d/helm-stable-debian.list

   # 4) Install
   _run_command --prefer-sudo -- apt-get update
   _run_command --prefer-sudo -- apt-get install -y helm
...
function _install_debian_docker() {
  ...
  _run_command --prefer-sudo -- apt-get install -y apt-transport-https ca-certificates curl software-properties-common
  # Add Docker's GPG key
  if [[ ! -e "/usr/share/keyrings/docker-archive-keyring.gpg" ]]; then
     _curl -fsSL https://download.docker.com/linux/"$(lsb_release -is \
        | tr '[:upper:]' '[:lower:]')"/gpg \
        | _run_command --prefer-sudo -- gpg --dearmor \
  ...
  echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/$(lsb_release -is | tr '[:upper:]' '[:lower:]') $(lsb_release -cs) stable" | \
     _run_command --prefer-sudo -- tee /etc/apt/sources.list.d/docker.list > /dev/null
  # Update package list
  _run_command --prefer-sudo -- apt-get update
  # Install Docker
  _run_command --prefer-sudo -- apt-get install -y docker-ce docker-ce-cli containerd.io
  # Start and enable Docker when systemd is available
  if _systemd_available ; then
     _run_command --prefer-sudo -- systemctl start docker
     _run_command --prefer-sudo -- systemctl enable docker
  else
     _warn "systemd not available; skipping docker service activation"
  fi
  # Add current user to docker group
  _run_command --prefer-sudo -- usermod -aG docker "$USER"
...
function _install_redhat_docker() {
  ...
  _run_command --prefer-sudo -- dnf install -y dnf-plugins-core
  # Add Docker repository
  _run_command --prefer-sudo -- dnf config-manager addrepo --overwrite \
     --from-repofile https://download.docker.com/linux/fedora/docker-ce.repo
  # Install Docker
  _run_command --prefer-sudo -- dnf install -y docker-ce docker-ce-cli containerd.io
  # Start and enable Docker when systemd is available
  if _systemd_available ; then
     _run_command --prefer-sudo -- systemctl start docker
     _run_command --prefer-sudo -- systemctl enable docker
...
function _ensure_cargo() {
   ...
   if _is_debian_family ; then
      _run_command --prefer-sudo -- apt-get update
      _run_command --prefer-sudo -- apt-get install -y cargo
   elif _is_redhat_family ; then
      _run_command --prefer-sudo -- dnf install -y cargo
